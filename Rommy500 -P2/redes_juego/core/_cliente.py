...
class ClienteMixin:
         ...
         while self.conectado:
             ...
             # Verificar que el socket existe y está conectado
                if not self.socket_cliente:
                   print("[RED-CLIENTE] Socket cliente es None, saliendo de recepción")
                   break
                ...
                print("[RED-CLIENTE] Conexión reseteada por el servidor.")
                break
             except ConnectionAbortedError:
                print("[RED-CLIENTE] Conexión abortada.")
                break
             except OSError as e:
                if hasattr(e, 'winerror') and e.winerror == 10038:  # Socket no válido
                   print("[RED-CLIENTE] Socket inválido o ya cerrado.")
                   break
                print(f"[RED-CLIENTE] Error OSError: {e}")
                break
             except Exception as e:
    ...
    # Al salir del bucle, asegurar que conectado sea False
         self.conectado = False
         print("[RED-CLIENTE] Hilo de recepción finalizado.")
    ...
    def sala_cerrada_por_host(self):
        """Método llamado cuando el host cierra la sala"""
        print("[CLIENTE] La sala ha sido cerrada por el host.")
        self.conectado = False
    ...
    # Evento para volver al menú principal
        if hasattr(self, 'un_juego') and self.un_juego:
         evento_py = pygame.event.Event(constantes.EVENTO_VOLVER_MENU_PRINCIPAL)
         pygame.event.post(evento_py)
