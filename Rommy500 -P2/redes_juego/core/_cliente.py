...
from redes_juego import unpack_message
    ...
    def _recibir_mensajes(self):
        ...
        while self.conectado:
            try:
                ...
                #Procesar mensajes completos (header 10 bytes + payload)
                while len(buffer) >= 10:  #HEADER_SIZE
                    #Intentar desempaquetar
                    message, error = unpack_message(buffer)
                    if error:
                        print(f"Error al desempaquetar mensaje: {error}")
                        #Si es error de payload incompleto, esperar más datos
                        if "incompleto" in error or "Payload incompleto" in error:
                            break
                        #Si es error grave, descartar y continuar
                        buffer = b""
                        break

                    header_len = 10
                    json_len = len(json.dumps(message).encode('utf-8'))
                    total_consumido = header_len + json_len
                    buffer = buffer[total_consumido:]                    
                
                    self._manejo_mensaje_red(message)
