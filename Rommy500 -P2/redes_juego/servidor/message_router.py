...
class MessageRouter:
    ...
    def route_message(self, id_jugador, mensaje, socket_cliente):
        ...
        if mensaje.get('type') == 'ClienteDesconectado':
           print(f"Mensaje de desconexión: {mensaje}")
        
             # Verificar que id_jugador sea válido
            if id_jugador is None:
               print("[SERVIDOR] Cliente desconectado sin ID asignado. Ignorando...")
               return
         
              # Verificar si el que se desconecta es el host
            with self.server.candado:
                es_host = False
                if self.server.clientes and len(self.server.clientes) > 0:
                   if self.server.clientes[0].get('id') == id_jugador:
                      es_host = True
            
                if es_host:
                    print(f"[SERVIDOR] El host (ID {id_jugador}) se ha desconectado. Cerrando sala...")
                    self.server.cerrar_sala_por_host_desconectado(id_jugador)
                    return
            
                 # Guardar datos del jugador desconectado
                nombre_cliente = "Desconocido"
                idx_cliente = id_jugador - 1
                if idx_cliente < len(self.server.clientes):
                   nombre_cliente = self.server.clientes[idx_cliente].get('nombre', 'Desconocido')
                ...
                print(f"Jugadores desconectados: {self.server.jugadores_desconectados}")
            
                   # Difundir desconexión (solo si el índice es válido)
                if idx_cliente < len(self.server.clientes):
                   self.server.difundir({
                       ...
                       "nombre": nombre_cliente,
                       "lista_jugadores": [c.get('nombre', '') for c in self.server.clientes]
                    })
                   print(f"Cantidad de clientes: {self.server.clientes}")
                
                 # Marcar como desconectado
                   self.server.clientes[idx_cliente]["status"] = "desconectado"
            
                 # Si estaba en partida, eliminar al jugador inactivo
                if self.server.estado_partida:
                   self.server.eliminar_jugador_inactivo(id_jugador)
            
                  # Verificar si hay menos de 2 jugadores activos después de la desconexión
                activos, _ = self.server.verificar_jugadores_activos()
                if activos < 2 and self.server.estado_partida:
                   print("[SERVIDOR] Menos de 2 jugadores activos. Reiniciando partida...")
                   self.server.reiniciar_partida_por_falta_jugadores()
            
            # Reanudar aceptación de conexiones si la partida estaba activa
                if self.server.estado_partida:
                   if not self.server.anunciar_servidor_estado:
        ...
        if mensaje.get('type') == 'Reconectar':
            ...
            # Si hay menos de 2 jugadores activos después de la desconexión
            activos, _ = self.server.verificar_jugadores_activos()
            if activos < 2 and self.server.estado_partida:
             print("[SERVIDOR] Menos de 2 jugadores activos. Reiniciando partida...")
             self.server.reiniciar_partida_por_falta_jugadores()
    
    # En lugar de eliminar el cliente, solo marcarlo como inactivo
            if id_jugador-1 < len(self.server.clientes):
