...
def validate_message(message: dict) -> tuple[bool, str]:
    """Valida un mensaje contra el diccionario maestro MESSAGE_SCHEMA.
    ...
    if msg_type not in MESSAGE_SCHEMA:
        logger.warning("Tipo de mensaje no registrado en schema: %s", msg_type)
        return True, ""
...
def unpack_message(sock: socket.socket, timeout: Optional[float] = None) -> tuple:
    """Desempaqueta un mensaje recibido por socket usando el header de 10 bytes.

    Returns:
        (message_dict, error_str)
    """
