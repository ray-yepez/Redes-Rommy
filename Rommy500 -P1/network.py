...
class NetworkManager:
    ...
    def clean_disconnect(self):
       try:
        self.running = False
        self.receive_thread_running = False
        # Avisar a otros jugadores (si es cliente)
        if self.player:
            try:
                # Enviar mensaje de salida ordenada
                self.exit_game(self.player_id, self.playerName)
            except Exception as e:
                print(f"Error al enviar mensaje de salida: {e}")
        # Cerrar conexiones de jugadores (si es host)
        for player in self.connected_players:
            try:
                if player[0] != self.server:
                    player[0].close()
            except Exception as e:
                print(f"Error cerrando conexión jugador: {e}")
        # Cerrar socket del servidor
        if self.server:
            try:
                self.server.close()
            except Exception as e:
                print(f"Error cerrando socket servidor: {e}")
        # Cerrar socket cliente
        if self.player:
            try:
                self.player.close()
            except Exception as e:
                print(f"Error cerrando socket cliente: {e}")
       except Exception as e:
        print(f"Error durante desconexión limpia: {e}")
